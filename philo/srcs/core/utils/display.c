...
{
	if (st == ST_EATING) return MSG_EAT;
	if (st == ST_SLEEPING) return MSG_SLEEP;
	if (st == ST_THINKING) return MSG_THINK;
	if (st == ST_DEAD) return MSG_DEAD;
	if (st == ST_PICKING_UP_FORK) return MSG_FORK;
	return "unknown";
}
...
t_bool	display_status(t_philo *ph, t_status st)
{
	...
	if (get_error_flag(ph->shared, ph->locks) || get_death_flag(ph->shared, ph->locks)
